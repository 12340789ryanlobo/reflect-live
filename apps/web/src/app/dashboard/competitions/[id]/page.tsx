...
export default function CompetitionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const statusLabel = comp.archived_at
    ? 'Archived'
    : comp.starts_at <= today && today <= comp.ends_at
      ? 'Live now'
      : comp.starts_at > today
        ? 'Upcoming'
        : 'Ended';
  const statusTone = comp.archived_at ? 'var(--ink-dim)' : statusLabel === 'Live now' ? 'var(--green)' : statusLabel === 'Upcoming' ? 'var(--amber)' : 'var(--ink-mute)';
        ...
        <section className="rounded-2xl border overflow-hidden" style={{ borderColor: 'var(--border)', background: 'var(--card)' }}>
          ...
          {rows.length === 0 ? (
            ...
            <table className="w-full border-collapse text-[13px]">
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={r.player_id} className="border-b last:border-b-0" style={{ borderColor: 'var(--border)' }}>
                    <td className="px-4 py-2.5 tabular text-[color:var(--ink-mute)] mono">{i + 1}</td>
                    <td className="px-4 py-2.5 text-[color:var(--ink)]">
                      <Link href={`/dashboard/players/${r.player_id}`} className="hover:text-[color:var(--blue)] transition">
                        {r.name}
                      </Link>
                    ...
                    <td className="px-4 py-2.5 text-right tabular font-bold text-[color:var(--ink)]">{r.points}</td>
                  </tr>
                ))}
              </tbody>
            </table>
